...
export class CoursesService {
    ...
    // Simuler une demande de paiement
  processPayment(method: string, courseId: string): Observable<any> {
    // Simule une réponse de succès après 2 secondes
    return of({ success: true, message: 'Payment processed successfully!' }).pipe(
      delay(2000)
    );
  }
